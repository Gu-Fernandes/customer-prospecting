"use client";

import { useState } from "react";
import Link from "next/link";

import { Button } from "@/components/button/button";
import { type Customer } from "@/services/customer.service";
import { EditCustomerModal } from "./edit-customer-modal";
import { useDisclosure } from "@/hooks/use-disclosure";
import {
  upper,
  formatCNPJDisplay,
  formatPhoneDisplay,
} from "@/utils/formatters";
import {
  Select,
  SelectTrigger,
  SelectContent,
  SelectItem,
} from "@/components/ui/select";
import { cn } from "@/libs/cn";

interface CustomersTableProps {
  customers: Customer[];
  onRowUpdated?: (c: Customer) => void;
  onRowDeleted?: (id: string) => void;
}

function ProductsCell({ products }: { products: string[] }) {
  if (!products || products.length === 0) return <span>—</span>;

  if (products.length === 1) return <span>{upper(products[0])}</span>;

  const count = products.length;

  return (
    <Select>
      <SelectTrigger size="sm">
        <span className="text-xs">Ver todos ({count})</span>
      </SelectTrigger>

      <SelectContent>
        {products.map((p) => (
          <SelectItem
            key={p}
            value={p}
            disabled
            className="pointer-events-none cursor-default select-none data-[disabled]:opacity-100"
          >
            {upper(p)}
          </SelectItem>
        ))}
      </SelectContent>
    </Select>
  );
}

function CustomerActionsBar({ className }: { className?: string }) {
  return (
    <div
      className={cn(
        "flex w-full items-center justify-between gap-2",
        className
      )}
    >
      <Link href="/customers/new">
        <Button variant="outline" className="w-full sm:w-auto">
          + Novo Cliente
        </Button>
      </Link>

      <Link href="/">
        <Button variant="default" icon="home" className="w-full sm:w-auto" />
      </Link>
    </div>
  );
}

function CustomersEmptyState() {
  return (
    <div className="text-sm text-zinc-500 dark:text-foreground/70">
      Nenhum cliente cadastrado ainda.
      <CustomerActionsBar className="mt-4 hidden gap-3 md:flex" />
    </div>
  );
}

function CustomersMobileList({
  customers,
  onEdit,
}: {
  customers: Customer[];
  onEdit: (customer: Customer) => void;
}) {
  return (
    <div className="space-y-3 md:hidden">
      {customers.map((c) => (
        <div
          key={c.id}
          className="rounded-lg border border-border bg-background p-3 shadow-md"
        >
          <div className="mb-2 flex items-start justify-between">
            <div>
              <p className="text-sm font-semibold text-foreground">
                {c.company}
              </p>
              <p className="text-xs text-zinc-500 dark:text-foreground/60">
                {formatCNPJDisplay(c.cnpj)}
              </p>
            </div>

            <Button
              variant="icon"
              size="sm"
              icon="pencil"
              iconPosition="only"
              aria-label={`Editar ${c.company}`}
              onClick={() => onEdit(c)}
            />
          </div>

          <div className="grid grid-cols-2 gap-2 text-xs">
            <div>
              <p className="text-zinc-400 dark:text-foreground/50">
                Responsável
              </p>
              <p className="text-zinc-700 dark:text-foreground/90">
                {c.responsible}
              </p>
            </div>

            <div>
              <p className="text-zinc-400 dark:text-foreground/50">Telefone</p>
              <p className="text-zinc-700 dark:text-foreground/90">
                {formatPhoneDisplay(c.phone)}
              </p>
            </div>

            <div className="col-span-2">
              <p className="text-zinc-400 dark:text-foreground/50">E-mail</p>
              <p className="break-all text-zinc-700 dark:text-foreground/90">
                {c.email}
              </p>
            </div>

            <div className="col-span-2">
              <p className="text-zinc-400 dark:text-foreground/50">Produtos</p>
              <div className="mt-0.5">
                <ProductsCell products={c.products} />
              </div>
            </div>

            <div>
              <p className="text-zinc-400 dark:text-foreground/50">SKU</p>
              <p className="text-zinc-700 dark:text-foreground/90">
                {c.sku ? upper(c.sku) : "—"}
              </p>
            </div>

            <div className="col-span-2">
              <p className="text-zinc-400 dark:text-foreground/50">
                Fornecedor
              </p>
              <p className="text-zinc-700 dark:text-foreground/90">
                {c.supplier || "—"}
              </p>
            </div>
          </div>
        </div>
      ))}
    </div>
  );
}

function CustomersDesktopTable({
  customers,
  onEdit,
}: {
  customers: Customer[];
  onEdit: (customer: Customer) => void;
}) {
  return (
    <div className="hidden w-full overflow-x-auto md:block">
      <table className="w-full border-collapse">
        <thead>
          <tr className="border-b border-border text-left text-[11px] font-medium uppercase text-zinc-500 dark:text-foreground/50">
            <th className="w-10 py-1 pr-1" />
            <th className="py-2 pr-4">Empresa</th>
            <th className="py-2 pr-4 whitespace-nowrap">CNPJ</th>
            <th className="py-2 pr-4 whitespace-nowrap">Responsável</th>
            <th className="py-2 pr-6 whitespace-nowrap">Telefone</th>
            <th className="py-2 pr-4">E-mail</th>
            <th className="py-2 pr-4">Produtos</th>
            <th className="py-2 pr-4 whitespace-nowrap">SKU</th>
            <th className="py-2 pr-4">Fornecedor</th>
            <th className="py-2 pr-4 whitespace-nowrap">Último contato</th>
          </tr>
        </thead>

        <tbody className="text-sm text-zinc-800 dark:text-foreground">
          {customers.map((c) => (
            <tr
              key={c.id}
              className="border-b border-zinc-100 hover:bg-zinc-50 dark:border-border/40 dark:hover:bg-background/40"
            >
              <td className="py-2 pr-2">
                <Button
                  variant="icon"
                  size="sm"
                  icon="pencil"
                  iconPosition="only"
                  title={`Editar ${c.company}`}
                  onClick={() => onEdit(c)}
                />
              </td>

              <td className="py-2 pr-4 font-medium text-zinc-900 dark:text-foreground">
                {c.company}
              </td>

              <td className="py-2 pr-4 whitespace-nowrap">
                {formatCNPJDisplay(c.cnpj)}
              </td>

              <td className="py-2 pr-4 whitespace-nowrap">{c.responsible}</td>

              <td className="py-2 pr-6 whitespace-nowrap">
                {formatPhoneDisplay(c.phone)}
              </td>

              <td className="py-2 pr-4">{c.email}</td>

              <td className="py-2 pr-4 align-middle font-medium">
                <ProductsCell products={c.products} />
              </td>

              <td className="py-2 pr-4 font-mono tracking-wide whitespace-nowrap">
                {c.sku ? upper(c.sku) : "—"}
              </td>

              <td className="py-2 pr-4">{c.supplier || "—"}</td>

              <td className="py-2 pr-4 whitespace-nowrap">—</td>
            </tr>
          ))}
        </tbody>
      </table>

      <CustomerActionsBar className="mt-4" />
    </div>
  );
}

export function CustomersTable({
  customers,
  onRowUpdated,
  onRowDeleted,
}: CustomersTableProps) {
  const { isOpen, open, close } = useDisclosure(false);
  const [selected, setSelected] = useState<Customer | null>(null);

  function handleOpenEdit(customer: Customer) {
    setSelected(customer);
    open();
  }

  function handleCloseEdit() {
    setSelected(null);
    close();
  }

  if (customers.length === 0) {
    return <CustomersEmptyState />;
  }

  return (
    <div className="w-full">
      <EditCustomerModal
        isOpen={isOpen}
        customer={selected}
        onClose={handleCloseEdit}
        onUpdated={(updated) => {
          handleCloseEdit();
          onRowUpdated?.(updated);
        }}
        onDeleted={(id) => {
          handleCloseEdit();
          onRowDeleted?.(id);
        }}
      />

      <CustomersMobileList customers={customers} onEdit={handleOpenEdit} />
      <CustomersDesktopTable customers={customers} onEdit={handleOpenEdit} />
    </div>
  );
}
